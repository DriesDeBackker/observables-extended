defmodule Observables.Obs do
  ...
  def zipn(obs) do
    ...
    indices = 0..(length(obs)-1)
    tagged = Enum.zip(obs, indices)
      |> Enum.map(fn {ob, index} -> ob
        #|> Observables.Obs.inspect()
        |> map(fn v -> {index, v} end) end)

    # Start our Zipn observable.
    {:ok, pid} = GenObservable.start(Zipn, [length(obs)])
  ...
  """
  def combinelatest2(l, r, opts \\ [left: nil, right: nil]) do
    ...
    {:ok, pid} = GenObservable.start(CombineLatest2, [left_initial, right_initial])
